...
`read.table` is part of `R`'s built-in `utils` package. It's the esiest way to import delimited data files, and results in a `data.frame`. Here's a `.csv` file containing data from my [Fitbit tracker](https://www.fitbit.com):

```{r}
theUrl <- "https//r.stuzog.com/docs/fitbit.csv"

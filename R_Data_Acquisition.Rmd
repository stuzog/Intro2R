...
**Create an RDS file** from the `fitbit` dataframe in memory:
...
**The `load()` function stores all the objects in an `RDS` file** with their names. The **`saveRDS()` function saves just one object** to a binary RDS file, but **without a name**, so when loading it with `readRDS` it must be assigned to an object.
